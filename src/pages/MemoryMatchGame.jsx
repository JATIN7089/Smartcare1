...
export default function MemoryMatchGame() {
  const { cognitiveProfile, updateProfileAfterGame, user, language } = useApp();
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs font-bold text-slate-600 bg-slate-100 px-3 py-1.5 rounded-full border border-slate-200 flex items-center gap-1.5">
            <Layers className="w-3.5 h-3.5 text-teal-600" /> Challenge: <strong className="text-teal-800">{currentDiff}</strong>
          </span>
      ...
      <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl text-center min-w-[70px]">
            <span className="text-[11px] font-bold text-slate-400 block uppercase">Attempts</span>
          ...
          >
            <span className="flex items-center gap-1"><HelpCircle className="w-3.5 h-3.5" /> Hint</span>
            ...
            >
              {isFlipped || isMatched ? (
                ...
                <div className="flex flex-col items-center justify-center gap-1 text-slate-400">
                  ...
                  <span className="text-[11px] font-bold text-slate-500">Tap to Reveal</span>
                </div>
              )}
            </button>
          );
      ...
      {isCompleted && gameResult && (
        <div className="bg-gradient-to-br from-emerald-50 via-teal-50 to-sky-50 border-2 border-teal-500 rounded-3xl p-6 shadow-xl space-y-5 animate-in zoom-in-95">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-black uppercase tracking-wider text-teal-800">Activity Completed!</span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="bg-white/90 rounded-xl p-3 border border-teal-100 text-center">
              <span className="text-xs text-slate-500 font-semibold block">Attempts</span>
              <span className="text-xl font-extrabold text-slate-800">{gameResult.attempts}</span>
            </div>
            <div className="bg-white/90 rounded-xl p-3 border border-teal-100 text-center">
              <span className="text-xs text-slate-500 font-semibold block">Time</span>
              <span className="text-xl font-extrabold text-slate-800">{gameResult.time}s</span>
            </div>
            <div className="bg-white/90 rounded-xl p-3 border border-teal-100 text-center">
              <span className="text-xs text-slate-500 font-semibold block">Activity Score</span>
          ...
          <div className="bg-white rounded-2xl p-4 border border-teal-200 space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-sm text-slate-900">Adaptive AI Adjustment</span>
              </div>
            ...
            {showExplanation && (
              <div className="text-xs text-slate-600 bg-slate-50 rounded-xl p-3 border border-slate-200 space-y-1">
                <p><strong>Explainable Model Rule:</strong> If accuracy &gt; 85% and response pace is brisk, difficulty gradually steps up to foster neuroplastic engagement. If mistakes accumulate, difficulty gently simplifies to prevent cognitive fatigue.</p>
